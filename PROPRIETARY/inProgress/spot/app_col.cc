...
fsm sens {
	...
	entry SE_0:
#ifdef SENSOR_LIST
		//AT May2013 read_sensor (SE_0, -2, &sens_data.sval[0]);
		sens_data.sval[0] = 0;
	entry SE_1:
...
#else
		read_sensor (SE_3, 1, &sens_data.sval[3]);
	...
	entry SE_DONE:
		...
		sens_data.sval[3] = pong_params.freq_maj;
		sens_data.sval[4] = pong_params.pow_levels;
		clr_alrms;
...
void do_butt (word b) {
	...
	if (ev & (1 << 2)) {
		switch (pong_params.freq_maj) {
			case 60:
				pong_params.freq_maj = 3600;
				break;
			case 3600:
